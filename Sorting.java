...
public static String charASorting (int e, int s, char [] array) { 
		...
		while (higherH>=lowerH) { //scan till lowerH and HigherH meet
			while(pvt < array[higherH]) {higherH--;} // end side -> if current element > pvt go on
			while(pvt > array[lowerH]) {lowerH++;} // start side -> if current element < pvt go on
		...
		if(e>lowerH) {charASorting(e, lowerH, array);} 
		
		if(higherH>s) {charASorting(higherH, s, array);} //Recursive calling to sort from start and end 
		}
...
public static void stringSortingS2 (int e, int s, String[] array) {
		...
		if(array != null) { //making sure array contains elements 
		while (higherH>=lowerH) { //scan till lowerH and HigherH meet
			while(0 < array[higherH].compareTo(pvt)) {higherH--;} // end side -> if current > origin element go on and stop when an element is < pvt
			while(0 > array[lowerH].compareTo(pvt)) {lowerH++;} // start side -> if current < origin element go on and stop when an element is > pvt
		...
		if(e>lowerH) {stringSortingS2(e, lowerH, array);} 
		
		if(higherH>s) {stringSortingS2(higherH, s, array);} //Recursive calling to sort from start and end 
		}
    }
}
